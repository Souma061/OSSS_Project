...
export function TestimonialsSection({ title = "Client Feedback", subtitle = "Hear what our clients say about our software solutions and services." }: { title?: string; subtitle?: string }) {
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {reviews.map((review, index) => (
            ...
            >
              ...
              <Typography variant="p" className="text-foreground/90 mb-10 flex-grow text-lg">
                &ldquo;{review.text}&rdquo;
              </Typography>
              <div className="flex items-center gap-4 mt-auto">
                <div className="h-14 w-14 rounded-full overflow-hidden">
                  <img loading="lazy" src={review.avatar} alt={review.name} className="w-full h-full object-cover" />
                </div>
                <div>
                  <Typography variant="large" className="font-semibold m-0">{review.name}</Typography>
                  <Typography variant="small" className="text-muted-foreground">{review.location}</Typography>
                </div>
              </div>
            </motion.div>
          ))}
